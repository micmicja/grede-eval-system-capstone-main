<x-layouts.app title="Record Projects">
    <div class="container-fluid py-4">
        <div class="row mb-4">
            <div class="col-md-8">
                <h2>Record Project</h2>
                <p class="text-muted">Add or update project scores for your students</p>
            </div>
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('project.store') }}">
                            ...
                            <div class="mb-3">
                                <label for="activity_title" class="form-label fw-bold">Project Title *</label>
                                <input type="text" class="form-control @error('activity_title') is-invalid @enderror"
                                    id="activity_title" name="activity_title" placeholder="e.g., Final Capstone Project"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <span class="input-group-text" id="totalLabel">/ {{ old('total_items', 100) }}</span>
                                </div>
            ...
            <div class="col-lg-7 mb-4">
                <div class="card shadow">
                    <div class="card-header bg-light">
                        <h5 class="mb-0">Recent Project Records</h5>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                'project')->orderBy('date_taken', 'desc')->limit(10)->get() as $project)
                                <tr>
                                    <td>{{ ucwords(strtolower($project->full_name)) }}</td>
                                    <td>{{ $project->activity_title }}</td>
                                    <td>{{ \Carbon\Carbon::parse($project->date_taken)->format('M d, Y') }}</td>
                                    <td>
                                        @if($project->score >= 80)
                                        <span class="badge bg-success">{{ $project->score }}/100</span>
                                        @elseif($project->score >= 60)
                                        <span class="badge bg-warning">{{ $project->score }}/100</span>
                                        @else
                                        <span class="badge bg-danger">{{ $project->score }}/100</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <form action="{{ route('project.destroy', $project->id) }}" method="POST"
                                                onsubmit="return confirm('Delete this project record?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
