<x-layouts.app title="Record Recitation">
    <div class="container-fluid py-4">
        <div class="row mb-4">
            <div class="col-md-8">
                <h2>Record Recitation</h2>
                <p class="text-muted">Add or update recitation scores for your students</p>
            </div>
        ...
        <div class="row">
            <!-- Recitation Form Card -->
            <div class="col-lg-5 mb-4">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">Add New Recitation</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('recitation.store') }}">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('activity_title') is-invalid @enderror"
                                    id="activity_title" name="activity_title" placeholder="e.g., Oral Report"
                            ...
                            <button type="submit" class="btn btn-primary w-100">
                                <i class="fas fa-save"></i> Save Recitation
                            </button>
                        </form>
                    </div>
                </div>
            ...
            <div class="col-lg-7 mb-4">
                <div class="card shadow">
                    <div class="card-header bg-light">
                        <h5 class="mb-0">Recent Recitation Records</h5>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                'recitation')->orderBy('date_taken', 'desc')->limit(10)->get() as $rec)
                                <tr>
                                    <td>{{ ucwords(strtolower($rec->full_name)) }}</td>
                                    <td>{{ $rec->activity_title }}</td>
                                    <td>{{ \Carbon\Carbon::parse($rec->date_taken)->format('M d, Y') }}</td>
                                    <td>
                                        @if($rec->score >= 80)
                                        <span class="badge bg-success">{{ $rec->score }}/100</span>
                                        @elseif($rec->score >= 60)
                                        <span class="badge bg-warning">{{ $rec->score }}/100</span>
                                        @else
                                        <span class="badge bg-danger">{{ $rec->score }}/100</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <!-- Update Button -->
                                            <a href="{{ route('recitation.edit', $rec->id) }}"
                                            ...
                                            <!-- Delete Button -->
                                            <form action="{{ route('recitation.destroy', $rec->id) }}" method="POST"
                                                onsubmit="return confirm('Delete this recitation record?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="5" class="text-center text-muted py-4">
                                        No recitation records found. Add one to get started.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
